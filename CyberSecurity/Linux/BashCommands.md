...
- `awk '<PATTERN> { <ACTION> }' <FILE>` : text-processing tool and programming language, used mostly with CSV files and tabular data.  
  ...
  Its name is the acronym of the 3 people in Bell Labs who created it in the 1970s.
```shell
...
- `bc` : interactive basic calculator that can evaluate math strings and return the result of the operation
  - `echo '1.5 + 2.3' | bc` : evaluate a math string from stdin


- `bzip2 <FILE>` : compress a file by bzip2 compression with extension `.bz2`, slower and more efficient than gzip
  - `-k` : keep the original file (by default it removes it)
  - `-v` : verbose mode
...
- `cut <FILE>` : extract from a file or stdin a part of the input
  - `-b 1-10` : bytes mode, extract bytes 1 to 10
  - `-c 1-10` : characters mode, extract characters 1 to 10
...
- `diff <FILE1> <FILE2>` : show the different lines between 2 files (`<` for content only in FILE1 and `>` for content only in FILE2)


- `dirname <FILE>` : show the directory of the file 


- `du <FILE OR FOLDER>` : display disk usage for a file or directory (split by sub-directory)
  - `-s` : only show a summary for a directory (instead of one line per sub-directory)
  - `-a` : show a line for each file of the directory (instead of one line per sub-directory)
...
- `head <FILE>` : display the first/last 10 lines of a file or stdin in the console (opposite of the `tail` command)
...
- `history` : show the history of commands executed in this terminal  
  ...
  - `-d 150` : delete command history entry at position 150 (in memory, saved to the file on shell exit)
```shell
...
- `man <CMD_NAME>` : display the manual of a command
...
- `sudo <CMD>` : run a command as superuser, require current user password and the user must be allowed to use sudo
```shell
...
su <USERNAME>                 # switch to a specific user (require password of the user to impersonate)
...
- `tail <FILE>` : display the first/last 10 lines of a file or stdin in the console (opposite of the `head` command)
  - `-n 5` : specify the number of lines to display
  - `-c 5` : specify the number of bytes to read instead
...
- `uniq` : remove adjacent duplicate lines in a sorted input
...
- `which <CMD_NAME>` : show the full path of the binary with a given name in the PATH
